...
export interface CoinDetection {
  ...
  confidence: number; // simple confidence proxy
}
...
export function detectCoin(
  ...
  try {
    ...
    cv.HoughCircles(
      ...
      circles,
      cv.HOUGH_GRADIENT,
      1.2,                          // dp (inverse ratio of accumulator)
 ...
 */
export function coinToScale(coin: CoinDetection, coinDiameterMm: number): number {
 ...
 */
export function isCoinFullyInFrame(
  coin: CoinDetection,
  frameWidth: number,
  frameHeight: number,
): boolean {
  return (
    coin.x - coin.radius > 0 &&
 ...
 */
export function drawCoinOverlay(
