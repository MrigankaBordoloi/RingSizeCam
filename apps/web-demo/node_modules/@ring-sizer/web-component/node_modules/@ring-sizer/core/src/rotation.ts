 ...
 */
export function estimateFingerTheta(
  ringMCP: Vec3,
  ringPIP: Vec3,
  middleMCP: Vec3,
): number {
  ...
  // Normalize the axis
  const axisNorm: Vec3 = scale(axis, 1 / axisLen);
  ...
  const projPerp: Vec3 = sub(ref, scale(axisNorm, refAlongAxis));
  ...
  const WORLD_PRIMARY: Vec3 = { x: 0, y: 0, z: 1 };
  const WORLD_FALLBACK: Vec3 = { x: 1, y: 0, z: 0 };
  const worldRef =
    Math.abs(dot(axisNorm, WORLD_PRIMARY)) > 0.9 ? WORLD_FALLBACK : WORLD_PRIMARY;

  const e1raw: Vec3 = sub(worldRef, scale(axisNorm, dot(worldRef, axisNorm)));
  const e1: Vec3 = scale(e1raw, 1 / length(e1raw));
  const e2: Vec3 = cross(axisNorm, e1);
 ...
 */
export function thetaCoverage(thetas: number[]): number {
