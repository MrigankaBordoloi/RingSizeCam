...
/**
 * Snap an interpolated size value to its nearest table entry's label.
 * Needed for non-numeric size systems (e.g. UK letter grades) where a raw
 * interpolated number is meaningless on its own — the caller wants the
 * closest real stocked size's display label instead.
 *
 * @param value Interpolated size value (as produced by circumferenceToSize).
 * @param table Ring-size lookup table (defaults to US chart).
 * @returns The label of the nearest entry by |entry.size - value|.
 */
export function nearestSizeLabel(value, table = DEFAULT_US_SIZE_TABLE) {
    if (table.length === 0) {
        throw new Error('Size table is empty');
    }
    let best = table[0];
    let bestDist = Math.abs(best.size - value);
    for (const entry of table) {
        const dist = Math.abs(entry.size - value);
        if (dist < bestDist) {
            best = entry;
            bestDist = dist;
        }
