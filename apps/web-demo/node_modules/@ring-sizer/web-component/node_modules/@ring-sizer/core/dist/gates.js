 ...
 */
export function checkGates(meta, config) {
    const reasons = [];
    // Gate 1: MediaPipe landmark confidence
    if (meta.landmarkConfidence < config.minLandmarkConfidence) {
        reasons.push(`Landmark confidence too low: ${meta.landmarkConfidence.toFixed(3)} < ${config.minLandmarkConfidence}`);
    }
    // Gate 2: Blur (variance of Laplacian)
    if (meta.blurScore < config.minBlurScore) {
        reasons.push(`Image too blurry: Laplacian variance ${meta.blurScore.toFixed(1)} < ${config.minBlurScore}`);
    }
    ...
    if (!meta.coinInFrame) {
        ...
        reasons.push(`${anchorLabel} detection confidence too low: ${meta.coinConfidence.toFixed(3)} < ${config.minCoinConfidence}`);
    }
    // Gate 4: Finger tilt (z-delta between ring-finger landmarks 13 and 14)
    if (meta.fingerTiltZ > config.maxFingerTiltZ) {
        reasons.push(`Finger tilt too steep: z-delta ${meta.fingerTiltZ.toFixed(4)} > ${config.maxFingerTiltZ}`);
    }
    return {
