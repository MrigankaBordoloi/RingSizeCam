...
export function toggleHud(root: Document | ShadowRoot): boolean {
  hudVisible = !hudVisible;
  const el = root.getElementById('debug-hud');
  if (el) {
 ...
 */
export function initHudToggle(root: Document | ShadowRoot): () => void {
...
export function renderHud(data: HudData, root: Document | ShadowRoot): void {
  const el = root.getElementById('debug-hud');
