 ...
 */
export const DEFAULT_US_SIZE_TABLE: SizeTableEntry[] = [
 ...
 */
export function circumferenceToSize(
  circumferenceMm: number,
  circumferenceRange: [number, number],
  table: SizeTableEntry[] = DEFAULT_US_SIZE_TABLE,
): MeasurementResult {
 ...
 */
function interpolateSize(circumferenceMm: number, table: SizeTableEntry[]): number {
  if (table.length === 0) {
  ...
  if (circumferenceMm <= first.circumferenceMm) {
    if (table.length < 2) return first.size;
  ...
  if (circumferenceMm >= last.circumferenceMm) {
    if (table.length < 2) return last.size;
    const secondLast = table[table.length - 2]!;
    const slope =
      (last.size - secondLast.size) /
  ...
  for (let i = 0; i < table.length - 1; i++) {
    ...
    if (circumferenceMm >= lo.circumferenceMm && circumferenceMm <= hi.circumferenceMm) {
      const t =
        (circumferenceMm - lo.circumferenceMm) /
