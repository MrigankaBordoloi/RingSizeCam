...
function interiorAngleDeg(a: Point2D, b: Point2D, c: Point2D): number {
 ...
 */
function scoreQuad(corners: [Point2D, Point2D, Point2D, Point2D]): number {
  ...
  const angleScore =
    angles.reduce((sum, a) => sum + (1 - clamp01(Math.abs(a - 90) / 90)), 0) / angles.length;
 ...
 */
export function isCardFullyInFrame(
  card: CardDetection,
  frameWidth: number,
  frameHeight: number,
): boolean {
 ...
 */
export function cardToScale(card: CardDetection, cardWidthMm: number, cardHeightMm: number): number {
 ...
 */
export function rectifyCardPreview(
  video: HTMLVideoElement,
  canvasForCapture: HTMLCanvasElement,
  card: CardDetection,
  outCanvas: HTMLCanvasElement,
): void {
  if (typeof cv === 'undefined' || typeof cv.Mat === 'undefined') return;
  ...
  let src: any = null;
  let srcQuad: any = null;
  let dstQuad: any = null;
  let M: any = null;
  let dst: any = null;
