 ...
 */
export function extractFingerWidth(landmarks, video, scratchCanvas, scale) {
    ...
    if (len < 5)
        return null; // landmarks too close, bad detection
    ...
    try {
        ...
        if (widthPx < 3)
            return null; // too narrow, likely noise
        return {
            widthPx,
 ...
 */
export function computeFingerTilt(landmarks) {
    const mcp = landmarks[RING_FINGER.MCP];
    const pip = landmarks[RING_FINGER.PIP];
    if (!mcp || !pip)
        return Infinity;
...
//# sourceMappingURL=widthExtractor.js.map